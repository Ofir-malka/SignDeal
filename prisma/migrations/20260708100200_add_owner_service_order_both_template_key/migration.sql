-- Migration: add OWNER_SERVICE_ORDER_BOTH to the ContractTemplateKey enum
--
-- Owner two-document family: the service-order agreement carries the fee terms
-- and is the primary document, resolved by (owner contractType + dealType BOTH).
-- Fee sides stay separate: rental amount from Contract.commission (+ MONTHS via
-- rentalCommissionMonths), sale amount from Contract.commissionSale (+ percent
-- via saleCommissionPercent).
--
...
ALTER TYPE "ContractTemplateKey" ADD VALUE IF NOT EXISTS 'OWNER_SERVICE_ORDER_BOTH';
